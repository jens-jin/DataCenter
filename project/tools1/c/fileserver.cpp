...
// 把文件的内容发送给对端。
bool SendFile(const int sockfd,const char *filename,const size_t filesize);
...
// 接收文件的内容。
bool RecvFile(const int sockfd,const char *filename,const char *mtime,size_t filesize);
  ...
  while (true)
  {
    ...
    if (strncmp(strrecvbuffer,"<filename>",10)==0)
    {
      ...
      UpdateStr(serverfilename,starg.clientpath,starg.srvpath,false);
       ...
       logfile.Write("(%ld) ...",filesize);
      // 接收文件的内容。
      logfile.Write("recv %s(%ld) ...",serverfilename,filesize);
...
bool RecvFile(const int sockfd,const char *filename,const char *mtime,size_t filesize)
{
  ...
  size_t  totalbytes=0;        // 已接收文件的总字节数。
  size_t  onread=0;            // 本次打算接收的字节数。
...
bool _tcpputfiles()
{
  ...
  while (true)
  {
    ...
    // 把文件的内容发送给对端。
    logfile.Write("send %s(%ld) ...",Dir.m_FullFileName,Dir.m_FileSize);
...
bool SendFile(const int sockfd,const char *filename,const size_t filesize)
{
  size_t  onread=0;        // 每次调用fread时打算读取的字节数。
  int  bytes=0;         // 调用一次fread从文件中读取的字节数。
  char buffer[1000];    // 存放读取数据的buffer。
  size_t  totalbytes=0;    // 从文件中已读取的字节总数。
  FILE *fp=NULL;
